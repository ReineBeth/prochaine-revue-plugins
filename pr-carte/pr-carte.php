 ...
 */
function render_block_pr_carte($attributes, $content) {
    // Extraction des attributs avec valeurs par défaut
    $title = $attributes['titleField'] ?? '';
    $subtitle = $attributes['subtitleField'] ?? '';
    $text = $attributes['textField'] ?? '';
    $image_url = $attributes['imageUrl'] ?? 'https://www.shutterstock.com/image-vector/vector-flat-illustration-grayscale-avatar-600nw-2281862025.jpg';

    // SVG pour l'icône de retournement
